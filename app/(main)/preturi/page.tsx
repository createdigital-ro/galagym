...
const Preturi = () => {
	return (
		<div>
			...
			/>
			<h1 className='text-4xl sm:text-5xl lg:text-6xl font-bold text-center'>Prețuri</h1>
			<div>
				<MembershipCategory title='Nelimitat' memberships={memberships.unlimited} />
				...
				<div>
					<h3 className='font-bold mt-8 text-center text-3xl md:text-4xl lg:text-[2.40rem]'>
						...
						<span className='bg-[#0f0f0f] text-yellow-400 px-1'>personalizate</span>?
					</h3>
					<MembershipCategory title='Antrenor personal' memberships={memberships.personalTrainer} />
				</div>
			</div>
		</div>
	);
...
const PriceContainer = ({ type, price, days, tag }: { type: string; price: number; days: number; tag: string }) => {
	return (
...
const MembershipCategory = ({ title, memberships }: { title: string; memberships: Memberships }) => {
	return (
		<div>
			<div className='lg:max-w-[700px] lg:mx-auto'>
				...
				<div className='grid grid-cols-1 md:grid-cols-3 md:max-w-[700px] md:mx-auto gap-4'>
					{memberships.map((m, index) => (
						<PriceContainer key={index} type={m.type} price={m.price} days={m.days} tag={m.tag} />
					))}
				</div>
			</div>
		</div>
